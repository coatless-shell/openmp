...
for arg in "$@"; do
    case $arg in
        --disable-r)
            ...
            ;;
        --help|-h)
            echo "OpenMP Compilation Checker"
            echo ""
            echo "Usage: $0 [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --disable-r    Skip R-specific tests (useful for non-R OpenMP usage)"
            echo "  --help, -h     Show this help message"
            echo ""
            echo "Tests performed:"
            echo "  1. OpenMP Installation (library and headers)"
            echo "  2. Compiler Verification (clang availability)"
            echo "  3. Compilation Testing (OpenMP compilation and execution)"
            if [[ "$DISABLE_R" != "true" ]]; then
                echo "  4. R Configuration (~/.R/Makevars setup)"
                echo "  5. Environment Variables (R session variables)"
            fi
            exit 0
            ;;
        *)
...
echo "=========================="
if [[ "$DISABLE_R" == "true" ]]; then
    echo -e "${YELLOW}Note: R-specific tests disabled (--disable-r)${NC}"
    echo ""
fi
...
if [[ "$DISABLE_R" != "true" ]]; then
    ...
    echo -e "${YELLOW}   > Skipping R-specific tests (sections 4-5) due to --disable-r flag${NC}"
fi
...
if [[ "$DISABLE_R" == "true" ]]; then
    echo -e "${YELLOW}Note: R-specific tests were skipped (--disable-r)${NC}"
    echo ""
fi
...
if [[ $TESTS_FAILED -eq 0 ]]; then
    echo -e "${GREEN}"
    if [[ "$DISABLE_R" == "true" ]]; then
        echo "[SUCCESS] OpenMP is properly installed for C/C++ development!"
    else
    ...
    echo -e "${RED}"
    echo "[FAILURE] COMPLETE FAILURE: OpenMP is not working at all."
    ...
    if [[ "$DISABLE_R" != "true" ]]; then
        ...
        echo "2. Re-run this checker: ./check-openmp.sh --disable-r"
    fi
    
    exit 1
else
    echo -e "${YELLOW}"
    echo "[WARNING]  PARTIAL FAILURE: OpenMP is partially working."
    ...
    if [[ "$DISABLE_R" != "true" ]]; then
        echo ""
        echo "For C/C++ development without R:"
        echo "  - Use: ./check-openmp.sh --disable-r"
    fi
